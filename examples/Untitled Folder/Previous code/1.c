...
static uint8_t last_target=0,suspect_c=0,suspect_send=0;
...
current_victim[0] = current_victim[1]=0;victim[0].is_rssi_stored=victim[1].is_rssi_stored=0;
...
{
    int i,j,new_node_add_flag=0,return_val=1;
    short int new_nbrs=0;
    uint8_t nodeid=0; 
    short int msg_instance=0;
    uint8_t temp_id,nbr_id;	
    uint16_t nbr_rank,diff;
   ...
   // if(!msg_instance == 100*nodeid)  //checking reprocessing of the info received by broadcast
    ...
    //if(nodes[i].neighbors!=new_nbrs)
    {
	//return_val=3;
    }
    nodes[i].neighbors=new_nbrs;
    printf("Node %u, R %u,P %u PR %u, Nbrs %d ",nodes[i].id, nodes[i].rank, nodes[i].parent_id, nodes[i].p_rank,nodes[i].neighbors);
    //storing neighbor
	...
	//MAPPER_GET_PACKETDATA(nbr_rank,appdata);	
	/*if(nodes[i].neighbor[j].id==nbr_id)
	{
		if(nodes[i].neighbor[j].rank>nbr_rank)
		{
		diff=nodes[i].neighbor[j].rank-nbr_rank;
		if(diff>256)
			printf("\nRank attack by %u old_rank %u new_rank %u\n",nodes[i].neighbor[j].id,nodes[i].neighbor[j].rank,nbr_rank);
		}
	}*/
	//nodes[i].neighbor[j].id=nbr_id;
        //nodes[i].neighbor[j].rank=nbr_rank;
...
static void addmyinfo(void)
{
	...
	printf("Node %u, X %d Y %d R %u,P %u PRank %u, Nbrs %d ",nodes[0].id, nodes[0].x, nodes[0].y, nodes[0].rank, nodes[0].parent_id, nodes[0].p_rank,nodes[0].neighbors);
...
{
	static int i=0,j=0,k=0,l=0,dist[15],distance1=0,node_loc[15],minrssi=0;
	static int attacker_found=0,attacker_count=0;	
	static uint8_t attacker[15],rank[15];
	PROCESS_BEGIN();
	printf("In postrssi \n");
	j=0;k=0;
	...
	{
		printf("Attacker %u count %d\n",attacker[i],rank[i]);
		attacker[i]=rank[i]=0;
	}
	attacker_count=0;victim_count=0;
...
PROCESS_THREAD(udp_server_process, ev, data)
{
	...
	{	
		...
		victim[0].is_rssi_stored=victim[1].is_rssi_stored=0;
		victim_count=0;
		current_victim[0]=0;current_victim[1]=0;
